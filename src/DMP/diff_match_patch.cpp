/*
 * QDiffX - Modern Qt6 Diff Algorithm & Widget
 *
 * High-performance text comparison and patch generation implementation
 * optimized for Qt6 applications with full Unicode support.
 */
...
diff_match_patch::diff_match_patch() :
  Diff_Timeout(0.0f),       // NO TIMEOUT - Force full precision
  Diff_EditCost(4),
  Match_Threshold(0.5f),    // Standard threshold for accuracy
  Match_Distance(100000),   // Much larger search distance for big files
  Patch_DeleteThreshold(0.5f),  // Standard threshold
  Patch_Margin(4),
  Match_MaxBits(8192) {     // Even larger pattern support for big files
}
...
int diff_match_patch::match_main(const QString &text, const QString &pattern,
 ...
 loc = std::max(0, std::min(static_cast<int>(loc), static_cast<int>(text.length())));
  if (text == pattern) {
...
int diff_match_patch::match_bitap(const QString &text, const QString &pattern,
  ...
  for (int d = 0; d < pattern.length(); d++) {
    ...
    int finish = std::min(static_cast<int>(loc + bin_mid), static_cast<int>(text.length())) + pattern.length();
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text) {
  ...
  while (text.indexOf(pattern) != text.lastIndexOf(pattern)
    ...
    pattern = safeMid(text, std::max(0, patch.start2 - padding),
        std::min(text.length(), static_cast<qsizetype>(patch.start2 + patch.length1 + padding))
        ...
        if (diff_type == INSERT) {
          ...
          // Deletion or equality.  Only take as much as we can stomach.
            diff_text = diff_text.left(std::min(diff_text.length(),
                                                static_cast<qsizetype>(patch_size - patch.length1 - Patch_Margin)));
          patch.length1 += diff_text.length();
          start1 += diff_text.length();
          if (diff_type == EQUAL) {
            patch.length2 += diff_text.length();
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
    ...
    text.removeAll("");  // Retirer les lignes vides
    Patch patch;
...
QString Patch::toString() const {
  ...
  if (length1 == 0) {
    coords1 = QString::number(start1) + ",0";
  } else if (length1 == 1) {
    coords1 = QString::number(start1 + 1);
  } else {
    coords1 = QString::number(start1 + 1) + "," + QString::number(length1);
  }
  if (length2 == 0) {
    coords2 = QString::number(start2) + ",0";
  } else if (length2 == 1) {
    coords2 = QString::number(start2 + 1);
  } else {
    coords2 = QString::number(start2 + 1) + "," + QString::number(length2);
  }
  QString text;
  ...
  foreach (Diff aDiff, diffs) {
    ...
    // Use toUtf8() to properly handle surrogate pairs
    text += QString(aDiff.text.toUtf8().toPercentEncoding(" !~*'();/?:@&=+$,#"))
        + QString("\n");
  }
